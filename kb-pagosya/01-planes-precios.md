---
title: Planes y precios PagosYa
version: v2
audiencia: merchants
actualizado_en: 2026-05-12
---

PagosYa ofrece planes mensuales o anuales, **sin comisión por venta**. Los precios actuales corresponden a la generación **v2 (2026)**. Los clientes con planes anteriores (v1) mantienen sus condiciones mientras su suscripción esté activa.

---

## Planes actuales (v2 — 2026)

### 🟣 EmprendeYa — Bs. 79 / mes *(Bs. 790 / año)*
> Precio original: ~~Bs. 129/mes~~ — **39% de descuento**

- Hasta **Bs. 40.000 en ventas / mes**
- 1 tienda · 1 empleado
- Sin límite de productos en catálogo
- Caja registradora básica
- Registro de ventas
- Confirmación automática de pagos QR
- **No incluye**: POS completo, facturas SIAT, gestión de clientes ni tienda online

---

### 🟢 ExpandeYa — Bs. 169 / mes *(Bs. 1.690 / año)* — ⭐ Más popular
> Precio original: ~~Bs. 249/mes~~ — **32% de descuento**

- Todo lo de EmprendeYa +
- Sistema POS completo
- Gestión de productos e inventario
- Sistema de facturas (SIAT)
- Gestión de clientes
- Hasta **Bs. 80.000 en ventas / mes**
- Hasta 3 tiendas · 3 empleados
- Hasta **2.000 productos** en catálogo
- **No incluye**: tienda online, checkout QR externo

---

### 🔵 ConquistaYa — Bs. 249 / mes *(Bs. 2.490 / año)*
> Precio original: ~~Bs. 369/mes~~ — **33% de descuento**

- Todo lo de ExpandeYa +
- Tienda Online con stock sincronizado
- Checkout con QR bancario
- Sincronización de stock entre tiendas
- Dashboard avanzado
- Hasta **Bs. 150.000 en ventas / mes**
- Hasta 6 tiendas · 6 empleados
- Hasta **5.000 productos** en catálogo

---

### 🌐 Plan Checkout Web — Bs. 249 / mes *(Bs. 2.490 / año)*
> Precio original: ~~Bs. 369/mes~~ — **33% de descuento**

- Pensado para negocios que integran pagos en su **propio sitio web**
- Checkout Externo / API de Pagos
- Integración con sitio web propio
- Dashboard de transacciones
- Confirmación automática de pagos QR
- Hasta **Bs. 150.000 en ventas / mes**
- 1 tienda · 1 empleado
- **No incluye**: POS, productos, facturas, tienda online

---

### 📅 CobraYa — Cobranza Recurrente *(NUEVO — 2026)*

> Plan especializado para negocios que cobran membresías, mensualidades o cuotas periódicas: academias, escuelas, guarderías, condominios, coworkings y clubes.

| Plan | Clientes | Mensual | Anual | Por cliente/mes |
|------|----------|---------|-------|-----------------|
| **CobraYa 300** ⭐ | Hasta 300 | **Bs. 199** | Bs. 1.990 | Bs. 0.66 |
| **CobraYa 500** | Hasta 500 | **Bs. 399** | Bs. 3.990 | Bs. 0.80 |
| **CobraYa 1000** | Hasta 1.000 | **Bs. 699** | Bs. 6.990 | Bs. 0.70 |
| **Corporativo** | +1.000 | **A negociar** | — | — |

**Incluye en todos los tiers:**
- ✅ Cobranza automática mensual con generación de cobros
- ✅ Recordatorios WhatsApp personalizables (antes, día y después del vencimiento)
- ✅ QR bancario para pagos (Baneco, BNB, Red Enlace)
- ✅ Portal del cliente con token seguro (sin contraseña)
- ✅ Dashboard con KPIs, gráficos y top deudores
- ✅ Gestión de grupos de clientes
- ✅ Recibos digitales con logo de la tienda
- ✅ Import/Export CSV de clientes
- ✅ Auditoría de cambios (anulaciones, vencimientos)
- ✅ Facturación SIAT

**No incluye:** POS, productos, inventario, tienda online ni API de checkout externo.

**💰 Comparativa vs pasarela de pago (1%):**
- 300 clientes a Bs.150 c/u: **CobraYa Bs.199/mes** vs pasarela Bs.450/mes = **56% de ahorro**
- 1.000 clientes a Bs.150 c/u: **CobraYa Bs.699/mes** vs pasarela Bs.1.500/mes = **53% de ahorro**

---

### 🍽️ RestauranteYa — Bs. 329 / mes *(Bs. 3.290 / año)*
> Precio original: ~~Bs. 499/mes~~ — **34% de descuento**

- Todo lo de ConquistaYa +
- **Modo Restaurante** incluido
- Gestión de mesas y comandas
- Envío de pedidos a cocina/barra
- Producción e impresión de comandas
- Hasta **Bs. 150.000 en ventas / mes**
- Hasta 6 tiendas · 6 empleados
- Hasta **800 productos** en catálogo

---

### 🏢 CorporativoYa — Precio a consultar
> Para negocios con **más de Bs. 250.000 en ventas / mes**

- Todas las funcionalidades de ConquistaYa o RestauranteYa
- Límite de ventas y de productos configurado a medida
- Soporte prioritario y acompañamiento dedicado
- Integraciones personalizadas disponibles
- Contactar al equipo de ventas PagosYa para cotización

---

## 💳 Créditos adicionales *(solo planes v1)*

- Cada crédito cuesta **Bs. 20**
- Permite:
  - Aumentar **+5.000 Bs de límite de ventas**, o
  - Agregar **1 tienda + 1 empleado adicional** (combo)
- Se compran desde el panel de suscripción
